...
def game_over_screen():
    # Keep the background and draw Game Over text in the center
    screen.blit(background, (0, 0))  # Keep the original background
    screen.blit(game_over_text, ((SCREEN_WIDTH - game_over_text.get_width()) // 2, (SCREEN_HEIGHT - game_over_text.get_height()) // 2))  # Center the text
    ...
    while True:
        for event in pygame.event.get():
            if event.type == QUIT:
                ...
                if event.key == K_SPACE:  # Restart the game by pressing Space
                    return True
                elif event.key == K_ESCAPE:  # Quit the game by pressing ESC
                    return False
