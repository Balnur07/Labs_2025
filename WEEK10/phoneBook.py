...
conn = psycopg2.connect(
    dbname="suppliers",
    user="postgres",
    password="2803",   # Или твой пароль от postgres
    host="localhost",
    port="5432"
)
...
# Запрос данных (по имени, телефону или всем записям)
def query_data():
    ...
    if filter_by == "name":
        ...
        cur.execute("SELECT * FROM phonebook WHERE LOWER(first_name) ILIKE LOWER(%s)", ('%' + name + '%',))
...
def delete_data():
    user_id = input("Enter the ID of the contact to delete: ")
    try:
